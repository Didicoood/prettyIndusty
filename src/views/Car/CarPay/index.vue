...
<script>
import { checkRuleAPI, createRuleAPI, delRuleAPI, editRuleAPI, getRuleListAPI } from '@/api/car'
...
export default {
  ...
  methods: {
    ...
    async  exportToExcel() {
      ...
      // 调整后端表头,去除id
      const sheetDate = res.data.rows.map(item => {
        const obj = {}
      ...
      // 创建一个工作表，对象数组格式
      const worksheet = utils.json_to_sheet(sheetDate)
      // 把工资表添加到工作普，data为工作表名称
    ...
    async handleEdit(id) {
      ...
      const res = await checkRuleAPI(id)
      this.addForm = res.data
      await editRuleAPI({ ...this.addForm, id })
      if (this.dialogVisible === false) this.addForm = {}
    },
    async handleDel(id) {
      await delRuleAPI(id)
      this.getRuleList()
